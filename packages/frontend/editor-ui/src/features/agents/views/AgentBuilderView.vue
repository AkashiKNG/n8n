...
import { N8nIcon, N8nResizeWrapper, type ActionDropdownItem } from '@n8n/design-system';
...
function onConfigFieldUpdate(updates: Partial<AgentJsonConfig>, meta?: { source: 'auto' }) {
	...
	// real edit or an assistant build instead.
	if (meta?.source === 'auto' && isUnsaved.value) return;
...
function scheduleExternalRefresh(onlyWhenIdle = false, source?: string) {
	...
	externalRefreshTimer = setTimeout(() => {
		...
		void refreshArtifactShell()
			.then(() => onAgentUpdateRefreshed(source))
			.catch(handleArtifactRefreshError);
	}, getDebounceTime(400));
}
