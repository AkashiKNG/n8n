 ...
 *
 * Router-free except for that one hand-off. Provides `SidebarStateKey` and the
 * thread runtime itself, so `InstanceAiViewHeader` / `InstanceAiConversation`
 * work unchanged.
 */
import {
	computed,
 ...
 */
const queuedHandoff = ref<{ context: InstanceAiHandoffContext; draft?: string } | null>(null);

function stashHandoff(
	threadId: string,
	queued: { context: InstanceAiHandoffContext; draft?: string },
) {
	...
	if (queued.context.source === 'agent-preview') {
		void store.updateThreadMetadata(threadId, {
 ...
 */
function handoff(context: InstanceAiHandoffContext, initialDraft?: string): boolean {
	...
	if (props.threadId) {
		stashHandoff(props.threadId, queued);
	} else {
...
provideSidebarState(sidebarCollapsed, sidebarWidth);
...
async function mintThread() {
	...
	try {
		...
		if (!mounted || props.subject.id !== mintedForSubjectId) {
			// Torn down, or switched subjects, while the mint was in flight — drop
			// the orphaned thread rather than adopt it under the wrong subject.
...
watch(
	...
	(next, previous) => {
		...
		if (!next) void resumeOrMintThread();
	},
);
...
function openFullAssistant() {
	...
	handedOffThreadId = props.threadId;
	void router.push({ name: INSTANCE_AI_THREAD_VIEW, params: { threadId: props.threadId } });
}

const currentThreadTitle = computed<string | undefined>(() => {
	if (!props.threadId) return undefined;
	const summary = store.threads.find((thread) => thread.id === props.threadId);
	const runtime = store.getRuntime(props.threadId);
...
<template>
	<div :class="$style.panel" data-test-id="instance-ai-embed-panel">
		<InstanceAiViewHeader :thread-id="threadId">
			<template #title>
